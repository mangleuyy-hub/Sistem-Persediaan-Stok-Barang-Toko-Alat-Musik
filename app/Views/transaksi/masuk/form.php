...
<div class="card"><div class="card-body">
    ...
    <form method="post" action="<?= $data ? site_url('barang-masuk/' . $data['id_masuk']) : site_url('barang-masuk') ?>">
        <?= csrf_field() ?><?php if ($data): ?><input type="hidden" name="_method" value="PUT"><?php endif; ?>
        <div class="row g-3">
            <div class="col-md-4"><label class="form-label">Tanggal Masuk</label><input type="date" class="form-control" name="tanggal_masuk" value="<?= esc(old('tanggal_masuk', $data['tanggal_masuk'] ?? date('Y-m-d'))) ?>" required></div>
        ...
        <div class="mt-3"><button class="btn btn-primary">Simpan</button><a class="btn btn-light" href="<?= site_url('barang-masuk') ?>">Kembali</a></div>
    </form>
</div></div>
<?= $this->endSection() ?>
